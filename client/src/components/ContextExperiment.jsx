...
export default function ContextExperiment({ cases = [] }) {
  ...
  async function run() {
    ...
    try {
      ...
      const res = await fetch('/api/sessions/impact/experiment', {
        ...
        body: JSON.stringify({ caseIds, runs, useCache: useCache && runs === 1 }),
      });
      if (!res.ok && !res.body) {
      ...
      for (;;) {
        ...
        for (const line of lines) {
          ...
          if (evt.type === 'start') setMeta({ caseCount: evt.caseCount, runs: evt.runs, model: evt.model });
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200/70 dark:border-gray-800 shadow-soft p-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 max-w-xl">
            Runs every eval case under a growing context stack — start at the bare user journey,
            add one layer at a time — to see where more context actually improves the analysis
            (and where it just costs tokens). Engine held constant (one-shot).
          </p>
        </div>
      ...
      {steps.length > 1 && <LayerCurve steps={steps} />}
                ...
                return (
                  <Fragment key={s.layer}>
                    ...
                    >
                      ...
                      <td className="py-1.5 px-3 text-right tabular-nums">{pct(s.f1)}</td>
